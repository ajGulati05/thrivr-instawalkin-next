...
import { MassageSpecialistsStyleWrapper } from '../MassageSpecialists/massageSpecialists.style';
...
import { formatDate, timezoneRegexFormat, changeDateFormat } from 'util/config';
...
export default function MassagePractice() {
  // Redux State
  ...
  const redirectHandle = async (query, projectIdData) => {
    ...
    if (payload) {
        ...
        getTeamTherapists(
          {
            ...
            team_slug: router.query.team_slug,
            date: date.toString(),
      ...
      gtagSearchParamsUpdate(
        router.query.city,
        date,
        durationValuePayload,
        status,
        therapistData
      );
    } else {
      let projectId = 0;
      projectIdData.map((element) => {
        if (element.default) {
      ...
      let date = new Date();
      setDateTime(date);
      setDurationValue(projectId);
      setLocationValue({ ...locationValue, city: router.query.city });
  ...
  const gtagSearchParamsUpdate = (
    ...
    window.dataLayer.push({
      event: 'searchTrigger',
      event_label: city.toUpperCase() + '_SEARCH',
      event_category: 'Search',
      data: {
        city: city,
        massage_date: date.toString(),
  ...
  const handleSearchSumbit = async (e) => {
    ...
    let { status, data: therapistData } = await dispatch(
      getTeamTherapists(
        {
          city: !isEmpty(locationValue.value)
            ? locationValue.value
            : defaultLaunchedCities[0].city_name,
          team_slug: router.query.team_slug,
  ...
  //   router.replace({
  //     pathname: `/massage-practice/${router.query.city}/${router.query.team_slug}`,
  //     query: {
  //       ...router.query,
  ...
  const handleMoreTime = (data) => {
    router.push(
      '/massage-therapists/[city]/[specialist]',
  ...
  return (
    <PageLoader visible={loaderCount <= 0 ? false : true}>
      <Head>
        ...
        <title>Massage Therapist - Thrivr</title>
      </Head>
